...
const CardEvent = ({ data }: { data: IEvent }) => {
  return (
      <VStack
        href={`/event/${data.contractAddress}`} passHref
        as={Link}
        spacing="1.6rem"
        h="full"
        w="full"
      ...
      >
        ...
        <VStack w="full" spacing=".2rem" alignItems="flex-start">
          <Heading fontSize="1.6rem" color="white" w="full" noOfLines={2} mb={4}>{data.title}</Heading>
          <Text fontSize="1.4rem" color="gray.300" w="full">{dayjs(data.start_date).format('ddd, MMM D • h:mm A')}</Text>
          <Text fontSize="1.4rem" color="gray.300" w="full" noOfLines={1}>{data.placeName}</Text>
          <Text fontSize="1.4rem" color="gray.300" w="full">{data.price} FTN</Text>
        </VStack>
      </VStack>
  )
}
