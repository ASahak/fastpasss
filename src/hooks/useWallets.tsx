...
const supportedWallets: Partial<Record<ConnectorsWithTypes, any>> = {
  [ConnectorsWithTypes.METAMASK]: {
    name: 'Metamask',
    icon: <Image width={40} height={40} alt="metamask connect" src={metamaskIcon} />
  },
  [ConnectorsWithTypes.YO_WALLET]: {
    name: 'YoWallet',
    icon: <Image width={40} height={40} alt="yowallet connect" src={yoWalletIconSrc} />
  },
}
  ...
  return useMemo(() => {
    ...
    const _wallets: IConnector[] = connectors
      .map((connector: Connector) => {
        const availableWallet = supportedWallets[
          ConnectorsWithTypes.YO_WALLET
